<template>
  <div>
    ...
    <el-dialog v-model="btnVisible" title="分配权限" destroy-on-close>
      <el-table
        ref="btnTableRef"
        :data="btnData"
        row-key="ID"
      ...
      >
        ...
        <el-table-column label="权限备注" prop="desc" />
      </el-table>
      <template #footer>
...
<script setup>
import { getDepartmentMenuTree, getMenuAuthority, addMenuAuthority } from '@/api/menu'
import {
  updateAuthority
} from '@/api/authority'
...
const init = async() => {
  // 获取所有菜单树
  const res = await getDepartmentMenuTree()
